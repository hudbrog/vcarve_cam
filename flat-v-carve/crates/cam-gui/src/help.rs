...
pub(super) fn explanation(label: &str) -> Option<&'static str> {
    ...
    Some(match name {
        ...
        "Between cuts" => {
            "What the V-bit does between two cutting excursions. Retract to clearance is the released behaviour: every excursion ends at the clearance plane above the stock top. Lift clear of the cut retracts only to the same clearance measured from the pass depth, which keeps the cycle but shortens the up-and-down travel. Route through the cut keeps the bit down wherever the shape itself allows the travel: it joins straight where the V-bit's cone at pass depth still fits inside the target — there the cut is one this stage already owes, so the join changes only the order of removal — and where it does not, a corner or a chord that would leave the shape, it travels around through the middle instead. Either way the join is taken only while it is quicker than the lift it replaces, and a route's every metre is checked by the same continuous sweep bound that checks every cut, so it can never leave the shape. Joining at depth leaves no clearance for a tool-length measurement error, which shows up as a mark along the join; on a fragile cutter the detours also engage the cone sideways at full depth. Retract to clearance avoids both."
        }
        "View controls" => {
