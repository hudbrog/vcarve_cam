...
mod tests {
    ...
    fn the_tool_group_offers_the_job_tool_and_library_actions() {
        ...
        // the tool it uses, the library picker, the copied profile's baseline
        // actions — the words differ, the element does not.
        for label in [
            "Change cutter tool",
            "Profile profiles",
        ...
        // Opening the picker reveals the library combos and the job's own
        // snapshots, exactly as in a Flat V-carve tool tab.
        app.operation_picker = Some(super::super::tool_picker::role_index(
            cam_core::project::v5::resources::AssignmentRole::Milling,
        ));
        let controls = render(&mut app, &ctx);
        for label in [
            "Profile assignment tool",
            "Clear cutter cutting values",
            "Browse library…",
        ] {
            assert!(controls.contains_key(label), "{label} missing");
        }
