...
describe('optional planning editors', () => {
  it('leaves all optional blocks unset on import without inferred presets', () => {
    const result = materialize(newDraft(parseJob(example)));
    expect(result.job?.endmill_planning).toBeNull();
    expect(result.job?.vbit_planning).toBeNull();
  ...
  it('retains partial planning separately and identifies missing settings by editable path', () => {
    ...
    expect(result.errors['endmill_planning.max_layers']).toMatch(/Complete/);
  ...
  it('keeps explicit zero cleanup iterations distinct from an empty required field', () => {
...
describe('machine profile and field presentation', () => {
  ...
  it('exposes units to assistive technology and omits inactive ramp controls', () => {
    ...
    expect(markup).toContain('aria-describedby="vbit_planning.quality_sample_spacing_mm-unit"');
  });
});
