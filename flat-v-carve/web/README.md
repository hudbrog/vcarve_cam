...
- Display-only placement, fit job/inspected region, mouse-wheel zoom centered on the pointer, button/keyboard zoom, pointer/keyboard pan, and physical axes. Placement uses `scale × rotate(page XY − origin)`; no machining calculations run in TypeScript.
