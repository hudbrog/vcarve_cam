...
import { editableDownloadAllowed, outputBlockedReasons, type ArtworkDisplay, type CamService, type Capabilities } from './contracts/service';
...
function Workspace({ initial, recovered, service, capabilities: initialCapabilities }: { initial: Draft; recovered: boolean; service: CamService; capabilities: Capabilities }) {
  ...
  const planningLabel = planning.active ? 'Background task active' : planning.task?.state === 'failed' ? 'Planning failed'
    : planning.current ? `Plan ${planning.result?.task.summary?.status}` : planning.result ? 'Previous plan stale' : 'Planning available';
  const planIssues = planning.task?.diagnostic ? [planning.task.diagnostic] : planning.current ? [
