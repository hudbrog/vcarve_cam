...
const pathControl = z.discriminatedUnion('kind', [
  z.strictObject({ kind: z.literal('exact_path') }),
  z.strictObject({ kind: z.literal('blend'), tolerance_mm: z.number().finite().min(0.000001).max(10),
    naive_cam_tolerance_mm: z.number().finite().min(0.000001).max(10).nullable() }),
]).default({ kind: 'blend', tolerance_mm: 0.05, naive_cam_tolerance_mm: 0.05 });
