...
export const planningLimitsSchema = z.strictObject({
  ...
  timeoutSeconds: integer.positive(), previewMotions: integer.positive(), artifactBytes: integer.positive().nullable(),
...
export const capabilitiesSchema = z.strictObject({
  ...
  toolLibrary:z.strictObject({schemaVersion:z.literal(1),maxBytes:integer.positive(),maxTools:integer.positive(),maxPresetsPerTool:integer.positive(),location:z.string()}).nullable().optional(),
  export: z.strictObject({ profileBytes: integer.positive(), programBytes: integer.positive(), layouts: z.array(z.enum(['combined','per_tool'])) }).optional(),
