...
const cacheName=`cam-gui1-assets-${self.GUI1_OFFLINE.version}`;
self.addEventListener('install',event=>event.waitUntil((async()=>{
  const cache=await caches.open(cacheName);await cache.addAll(self.GUI1_OFFLINE.assets);await self.skipWaiting();
})()));
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  const previous=(await caches.keys()).filter(n=>n.startsWith('cam-gui1-assets-')&&n!==cacheName);
