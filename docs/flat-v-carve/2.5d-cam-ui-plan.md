...
Every draggable tab/start/rectangle also has numeric or list controls. Every icon has a name/tooltip, every custom selectable element has appropriate accessible information, and every warning can be read as text. Test keyboard navigation, focus restoration, IME, high DPI, and representative screen-reader behavior separately on native and web; do not promise parity solely because the pixels match.
...
4. Portable open/save, recovery failure handling, multi-file download, clipboard, focus/IME, and representative accessibility checks on both targets.
