...
Follow-up on the real flower job (`flower_box-svg.job-real.json`), after a user report of `PLAN_WORKER_FAILURE`: the current bundle plans it end-to-end in the browser — **combined planning complete with 22,893 recorded motions (22,161 cutting) and the full paged preview, followed by an M5 verification that Passed with zero findings and zero unresolved cells**, matching the native CLI result. The reported failure was traced to a rebuild of `web/dist` racing the live session (the compute child's script 404'd mid-use); the node-side harness on the same artifact plans the flower job in ~10 s at ~235 MB RSS, and worker-failure diagnostics now include the underlying error and a reload hint. Rebuilding the bundle while a session is open remains a development-only hazard; deployed static hosting replaces files without emptying the directory.
