...
| Browser scenario — **landed** | `crates/cam-gui/web/gui10-scenario.mjs`: the facing job carries a machine clock (89.94 s over 31 motions, a rapid rate the machine does not state), 1× advances *inside* one pass (eight samples, fraction 0.080 → 0.487, feed 2400 mm/min), every sample draws its cutter on the move the material is being cut by (`sceneTip` is `planTip` normalized, and the marker's tool is the stream's own), Fit runs the program in 20 s at 4.5×, a tool that states no assembly is not invented, and the tight-holder fixture reports the ER20 nut 0.7 mm inside the material from motion 19 on a 0.4 mm raster with a row that seeks to it. Run with `node crates/cam-gui/web/smoke.mjs --gui10`; the recorded numbers and screenshots land in `artifacts/gui/browser-smoke/`. It found three defects no unit test did: a seek dropped the display's clock, the assembly readout was empty at the end of the program, and the cutter marker was drawn in plan millimetres instead of scene units (a marker "completely unseen" with a trail running off the viewport) |
...
| Probe fields | `simulation: { timed, prefix, fraction, elapsedSeconds, totalSeconds, tip, sceneTip, planTip, bounds, feedMmMin, rapidRateMmMin, rapidRateAssumed, planMotions, displayMotions, markerToolId, stageToolId, assembly, holderSegments }` beside the existing `stockPrefix` / `requestedStock` / `stockTransferBytes`; `sceneTip` / `planTip` are the tip the last drawn frame used, in scene coordinates and in plan millimetres, so a review compares one frame rather than the drawn overlay against a clock that has already advanced |
