...
browser screen-reader access as a requirement. The observed browser AccessKit
gap is an accepted scope limitation, so egui evaluation continues. Keyboard
navigation, focus, input handling, native accessibility and the remaining GUI1
probes still apply; GUI2 production work has not started.
...
| --- | --- | --- |
| Widget/application shell | egui + eframe | Dense forms, stable widget IDs, raw text, focus, IME and keyboard navigation on both targets; native accessibility. Browser screen-reader access is outside the required scope. |
...
8. Keyboard-only navigation of the critical flow, focus restoration after a dialog, clipboard and UI scaling on both targets, plus representative native platform screen-reader behavior. Browser screen-reader access is not required.
...
| Required accessibility | Named controls and usable critical workflows in the framework harness/native accessibility surface; keyboard navigation and focus on both targets; native screen-reader behavior | Identify toolkit vs custom-widget gaps and reassess required failures. Browser screen-reader access is an accepted exclusion. Do not equate an internal text-to-speech option with native screen-reader parity. |
