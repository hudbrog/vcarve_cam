...
Date: 2026-09-12\
Status: GUI1–GUI5 are complete; the user confirmed GUI5 completion on 2026-09-12. GUI6 passive drag knife, GUI7 facing/ordered preparation and GUI8 profiling (closed profile, tabs, radial finishing, starts/entries) are implemented and ready for manual review; user acceptance is pending. GUI9 larger jobs and deeper inspection — larger-job playback (GUI9a), finer stock inspection (GUI9b) and display robustness under load (GUI9c) — is implemented and ready for manual review. The GUI1 framework decision remains accepted with explicit scope limits (Windows x86_64 native and desktop Chromium/WebGPU).\
Scope: a new shared desktop/browser workspace for the existing Rust CAM engine.

Implementation evidence: [GUI1 framework decision](ui-framework-decision.md) and
[new UI progress](ui-implementation-progress.md). On 2026-09-11 the user removed
browser screen-reader access, then extended that exclusion to native screen readers.
Screen-reader support is not required on either target. The observed browser
AccessKit gap remains an accepted limitation. Keyboard navigation, focus, IME and
named controls for framework tests are covered by the GUI1 evidence; the checks
GUI1 deferred (real OS IME tour, real drag gesture, file-dialog confirm/cancel,
real device loss, sustained frame-time and browser-total memory, plus the
Firefox/Linux/Safari rows) are owned by the later slices named in the
[framework decision](ui-framework-decision.md#outcome-proceed-with-explicit-scope-limits).
GUI2 production work lives in `flat-v-carve/crates/cam-gui`; see the
[GUI2a implementation report](gui2a-progress.md). The experiment is historical
evidence, not the production runtime. Native and browser entry points launch
the new GUI directly. The former React workspace UI has been removed, so
`cam-gui` is the only UI.
The user permits breaking old saved jobs and machine settings: this slice uses
the current schemas directly and does not require a runtime legacy bridge.
...
The first milestone, **GUI1: framework selection and risk reduction**, selected egui/eframe with a wgpu viewport. The user accepted that framework within the documented Windows/Chromium scope. GUI2 implements the production application in `crates/cam-gui`; the following boundaries govern its incremental development.
...
| [CAM implementation checklist](2.5d-cam-checklist.md) | Evidence of actual backend and historical UI implementation. A planned field or successful screenshot is not completion evidence. |
...
Use `GUI1`, `GUI2`, etc. for this plan. Do not reuse the historical web UI's U-series or the backend A–H labels. Completing the Rust-only F3 does not complete GUI6's knife interface. Completing GUI1 does not complete any H slice. This delivery revision replaces the earlier horizontal GUI2–GUI10 breakdown; those unimplemented slice names are not parallel work queues. The 2026-09-12 reorder moves knife-only work from former GUI8a/b to GUI6a/b, facing from GUI6 to GUI7, profiles from GUI7 to GUI8, and mixed knife sequences from former GUI8c to GUI7d. Historical progress reports retain their original milestone references; this plan governs the current sequence.
...
The workspace currently has `cam-core`, pure `cam-service` DTO/projection code, the `cam-gui` application, the `cam-app` CLI, `cam-server`, and `cam-storage`. Preserve their domain responsibilities. `cam-app` is not an unused name for a new GUI crate.

The removed React workspace (the former `web/`, retained in Git history) used typed contracts, worker supervision, recovery behavior, simulation fixtures and a bug history that were useful engineering references during the migration; the behavior that survived lives in `crates/cam-gui`. Its component hierarchy and appearance were not the new design's starting point, and components were never ported mechanically into an egui function.

The former browser worker separated a parent ledger from disposable compute children. However, existing sequence commands do not by themselves establish the H5 retained-sequence contract. Likewise, the retired TypeScript sequence simulator had a display motion cap and a milling-tool assumption; these were limitations to address, not requirements to reproduce in the product.
...
| --- | --- | --- |
| Widget/application shell | egui + eframe | Dense forms, stable widget IDs, raw text, focus, IME and keyboard navigation on both targets; named controls for framework tests. Native and browser screen-reader support is outside the required scope. |
...
8. Keyboard-only navigation of the critical flow, focus restoration after a dialog, clipboard and UI scaling on both targets. Native and browser screen-reader support is not required.
...
| Required input and testability | Named controls and usable critical workflows in the framework harness; keyboard navigation and focus on both targets | Identify toolkit vs custom-widget gaps and reassess required failures. Native and browser screen-reader support is an accepted exclusion. |
...
| Testing | Framework tests can find named controls and detect a broken command/focus transition; real browser tests exercise keyboard/pointer behavior | Establish a testable UI surface before adding many bespoke controls. Browser accessibility-tree exposure is not required by this test criterion. |
...
`crates/cam-gui/src/{worker,platform,compute}.rs` implements this arrangement; the former `web/src/service/wasm/{worker,child,ledger,transport}.ts`, retained in Git history, is the additional engineering reference. H5 must extend the sequence artifact contract; merely wrapping the existing synchronous sequence route in a Promise is insufficient.
...
The preview camera is **one orthographic orbit** shared by the GPU renderer, the CPU picker and every screen-space overlay (`cam-gui/src/camera.rs`): azimuth (`yaw`), elevation (`tilt`; `0` is the top view, the historical isometric mix `0.65*v + 0.76*z` is `ISO_TILT`, and `±85°` is the limit that keeps the ground-plane pointer conversion solvable), `zoom`, and a view-target `pan` in the same normalized scene units the vertex payload uses, applied before the zoom so a panned point stays put when the zoom changes. `scene.wgsl` and `stock.wgsl` read `Camera::uniform`; the picker and the setup-coordinate conversions invert it. Nothing else may re-derive the projection.

Saved views carry the elevation as `tilt_deg` and keep the historical `isometric` flag for readers and review scenarios written before free rotation; a view without `tilt_deg` restores the elevation that flag records. The pointer map is: primary drag orbits, middle drag (or Shift with primary) pans, the wheel (or a pinch/ctrl-wheel gesture) zooms toward the pointer, and Top / Isometric / Front / View / Fit set the elevation or re-frame the scene. Edit handles and selectable geometry own their drags first (section 9.3), so navigation never steals an active gesture, and the camera remains display-only: it enters neither the job nor the machining identities.
...
The stock's own display model — interior walls derived as step instances, per-cell operation/tool identity, colouring modes (by operation, by tool, by depth with a gradient on walls), artwork/path toggles and a translucent stock — is planned in [stock-display-plan.md](stock-display-plan.md). Its contracts (identity packing, wall records, the display-only `StockStyle`, and one shading model for floors and walls) are what the viewport slices above must not contradict.
...
The simulator is the narrow renderer-independent Rust port that GUI1 selected: `crates/cam-gui/src/sim.rs` with the shared setup policy in `crates/cam-gui/tests/support/sim_setup.rs`. The retired TypeScript implementation (the former `web/src/sim/{engine,sequenceSim,store}.ts` with its tests, retained in Git history) is the reference it preserves: endmill/V-bit update, quantization, sampling and checkpoint meaning, checked with side-by-side fixture comparisons. A port needs its own evidence; confidence in the original implementation is not proof that the port is correct.

Compare matched grids, motion positions and stock samples against the recorded cross-check fixtures, not just similar screenshots. Add independent small numerical fixtures so identical mistakes are not accepted solely through parity. Generalize narrow tool/stage assumptions when the workflow needs it, with checked indices throughout; a knife-only job must not require a milling tool.
...
GUI1–GUI5 are complete. GUI6a/b, GUI7a–d, GUI8a–d and GUI9a–c are implemented and ready for manual review; GUI10–GUI11 remain Not started. See [the GUI6 report](gui6-progress.md) and [review recipe](gui6-review.md), [the GUI7 report](gui7-progress.md) and [review recipe](gui7-review.md), [the GUI8 report](gui8-progress.md) and [review recipe](gui8-review.md), [the GUI9 report](gui9-progress.md) and [review recipe](gui9-review.md), alongside [the GUI5 report](gui5-progress.md) and [post-review experience update](gui5-experience-update.md). The user-visible release unit is a vertical slice such as GUI2a or GUI8b. Each includes the necessary work across the architecture and leaves a runnable application. Module creation, pure tests and refactoring are internal tasks, not separate acceptance milestones. GUI1's experiments were the initial exception: they inform framework selection and provide an early interaction review, without pretending to be a usable CAM release.
...
| GUI6 passive knife cutting | GUI5 | Create a knife-only job, inspect planned/emitted traces and export/reopen | F3, qualified open-chain/closed-contour references and current H4/H5 evidence/output transport; facing and profiling are not prerequisites. |
| GUI7 facing and ordered preparation | GUI5; GUI6 for GUI7d | Face stock, adjust coverage, use that result before a known Flat V-carve and inspect supported milling/knife sequences | Implemented Face, resolved heights/stock history, H5 scoped sequence preparation and F3 contact/evidence for GUI7d. **Implemented and ready for manual review** (GUI7a–d); see [the GUI7 report](gui7-progress.md) and [review recipe](gui7-review.md). |
| GUI8 profiling and workholding | GUI7c | Cut selected profiles, then add tabs, finishing and entries in separate usable increments | Implemented Profile/tabs/finish/entries and their inspection contracts; GUI7d is not required. **Implemented and ready for manual review** (GUI8a–d); see [the GUI8 report](gui8-progress.md) and [review recipe](gui8-review.md). |
| GUI9 larger jobs and deeper inspection | GUI4 | Scrub large jobs responsively and inspect finer stock/features | Retained pages and bounded display work; no prerequisite for basic simulation in GUI2. **GUI9a–c implemented and ready for manual review**; see [the report](gui9-progress.md) and [review recipe](gui9-review.md). |
| GUI10 advanced output delivery | GUI7c; GUI6 for knife-specific cases | Save ordered sequential bundles and inspect prepared-program positioning | H5 complete ordered-bundle contract; F3 for knife-specific cases. |
| GUI11 usability and release qualification | GUI6, GUI7, GUI8, GUI9, GUI10 | Use the complete workflow in reviewed, qualified native/browser packages | H6 plus the software and manual-review evidence accumulated throughout delivery. |

The table is a default order, not a reason to postpone an independent user priority. The 2026-09-12 revision prioritizes standalone drag knife immediately after GUI5. Ordered mixed sequences remain in GUI7 because they need operation-prefix UI and stock-history integration; they do not block GUI6 acceptance. GUI9 can improve a large Flat V-carve job while new operations are still being evaluated. GUI2 and GUI3 must not wait for the general collection/resource roadmap to finish. Public backend subcontracts must still be defined and tested before a slice relies on them.
...
GUI1's four stages above are complete (reference inventory, interaction
prototype, runtime/simulation feasibility and the framework decision), and the
user accepted the outcome on 2026-09-11. The accepted envelope, the deferred
checks with their owner slices, and the one revised performance budget are
recorded in the [framework decision](ui-framework-decision.md#outcome-proceed-with-explicit-scope-limits).
...
### 14.8 GUI6: passive knife cutting

Technical status: GUI6a/b implemented, with native/browser review packages and
automated/software smoke evidence in [the report](gui6-progress.md). User review
is pending; use [the review recipe](gui6-review.md). The requirements below remain
the acceptance scope and do not imply physical blade-tracking qualification.
...
**GUI6b — corner and entry inspection.** Add detailed swivel/alignment/depth/closure readouts and navigation in that workflow. Manual review: inspect a corner and disconnected-chain entry, change supported settings, then change output precision/work zero and verify the old emitted replay becomes stale. Distinguish planned evidence from actual-byte replay and missing samples from checked ranges.

Operation lifecycle is included in GUI6: delete the current operation, retain the
job context in a saveable empty job, and add a Flat V-carve or drag-knife
operation with Undo/Redo. Adding starts the new operation's cutting inputs and
selection unset. The one-operation limit remains explicit until GUI7.

Knife authoring also exposes operation-owned geometry checkboxes and viewport
path assignment, plus an explicit stroked-copy action for filled boundaries.
Reviewable knife/profile-derived pass-depth suggestions and labeled starting
values fill blank fields without overwriting edits; physical heading remains
explicit. See the GUI6 review recipe for formulas and source-copy semantics.

GUI6 depends on GUI5 and the implemented F3/H4/H5 contracts, not on facing, profiling or operation-prefix UI. Include supported open-chain import/selection, typed knife creation/editing, knife tool/profile application and reset, knife-only stock initialization, timeline trace/orientation display, and planned versus actual-byte evidence freshness in this slice. Reuse the existing resource, worker, save and checked-output paths; extend their milling-only assumptions where needed. A knife form without playback is an internal task, not GUI6a acceptance. Supported mixed sequences are delivered in GUI7d after ordered preparation exists.

### 14.9 GUI7: introduce facing and ordered preparation

Technical status: GUI7a–d implemented, with native/browser review packages and
automated/software smoke evidence in [the report](gui7-progress.md). User review
is pending; use [the review recipe](gui7-review.md). The requirements below
remain the acceptance scope and do not imply any physical surface or
blade-tracking claim.

**GUI7a — face a stock by itself.** Add a source-free Face job with stock/tool/cutting settings, generated paths, actual heightfield playback and checked export. Manual review: configure a small fixture, change its facing depth, inspect original/final stock and reopen. Automated coverage is a prerequisite; record this as a new operation under user evaluation, not as already battle-tested because it has core tests.

**GUI7b — control facing coverage.** Add rectangle/whole-stock modes, margins, travel overrun and supported pass controls in the same loop. Manual review: change the area and extension, inspect what is actually removed and where the tool travels, and check a rejected setting routes correctly. Distinguish requested coverage from overhang; source geometry remains optional.

**GUI7c — face before the familiar carving.** Expose adding/reordering/enabling operations, height dependencies, prefix generation and one-program prefix export. Manual review: Face → Flat V-carve, inspect stock after each operation, hide earlier paths without losing their removal, change face height, then deliberately reorder across the dependency and repair it. H5 must bind the selected scope through preparation; the current whole-job export route is not a valid prefix shortcut.
...
**GUI7d — supported mixed milling/knife sequences.** After GUI7c and GUI6, use a supported Face → Drag knife fixture, inspect each operation's stock context, and exercise unsupported-contact/reversal feedback. Reuse the GUI6 knife editor and evidence views with operation-prefix generation, playback and checked export; knife traces leave the preceding stock unchanged. Include a rejected planted turn in cleared material with a located explanation. Respect F3's conservative contact checks and current evidence limits; arbitrary prior carving/profile removal is not assumed supported. Manual acceptance does not assert physical blade tracking or new alignment capabilities beyond F3.

### 14.10 GUI8: introduce profiles, then workholding and finish controls

Technical status: GUI8a–d implemented, with native/browser review packages and
automated/software evidence in [the report](gui8-progress.md). User review is
pending; use [the review recipe](gui8-review.md). The requirements below remain
the acceptance scope and do not imply any physical surface or edge claim.

| Slice | New usable capability | Manual acceptance tour | Required work/evidence |
| --- | --- | --- | --- |
| GUI8a basic closed profile | Create a profile alone or after the known carving | Select outer/hole contours, set explicit sides/depths, simulate the cut and export/reopen | Qualified contour table, offset/direction/height inspection, appropriate stock display; current backend algorithms and rejection cases. |
| GUI8b tabs | Retain visible material bridges in that actual cutting workflow | Add/move/remove tabs numerically and by dragging; scrub rough and final passes; export and reopen | Source anchors, candidate versus generated geometry, single-gesture Undo and visible sub-cell display limits. |
| GUI8c finishing | Leave radial allowance and run the configured finish pass | Compare rough/finish stock and feeds, change allowance, regenerate with tabs still present | Backend finish semantics, per-pass inspection and assignment preservation; no UI-side path construction. |
| GUI8d starts and entries | Control where/how the real profile enters and leaves | Move a start, choose supported leads/ramp, inspect motion, then provoke and repair an invalid entry | Typed candidate commands, source revision binding, lead/tab constraints and located errors; every drag has numeric/list equivalents. |

Each letter is a complete usable addition, with generated/simulated/exported results. Do not batch all tab, finish and entry infrastructure before presenting GUI8a. Record actual user/physical experience separately from software fixture results.

### 14.11 GUI9: larger jobs and deeper inspection

Technical status: GUI9a (larger jobs and responsive playback), GUI9b (finer
stock inspection) and GUI9c (robust display under load) implemented, with
native/browser review packages and release measurements on a 137,717-motion
batch fixture in [the GUI9 report](gui9-progress.md); user review is pending,
use [the review recipe](gui9-review.md). The requirements below remain the
acceptance scope and do not imply any physical surface or edge claim.
...
**GUI11c — controlled cutover.** Switch the preferred entry point after required technical and user-review issues are resolved. Preserve recovery/files and a clear compatibility route. Older clients cannot silently down-convert collection jobs. Removing the incumbent route is a separately reviewable change with migration/rollback evidence; the React route itself was removed from the repository once `cam-gui` became the sole UI.
...
| Async execution, cancellation, retained checked bytes | GUI2a single-job contract | New-format integration in GUI4, scopes in GUI7, bundles in GUI10. |
...
| --- | --- | --- |
| Toolkit fails required input/testability behavior | Prototype real critical flows early; evaluate the precise failing boundary before writing many controls | GUI1 |
...
| Old result appears current after edit/open/restart | Separate document epoch, instance, request generation, scope and semantic identities | GUI2; expand with GUI4, GUI6 evidence and GUI7 scopes |
...
| Backward scrub or path hiding loses earlier removal | Roughing/V-bit history first, then operation-prefix checkpoints and replay equivalence tests | GUI2, GUI7, GUI9 |
| Coarse simulation misrepresents tabs or holes | Visible cell size and required overlays when profiles ship; finer inspection later | GUI8, GUI9 |
| Knife display looks plausible while evidence is stale | Separate source/hash identity and consume F3 samples rather than recomputing tracking | F3, GUI6; mixed contexts in GUI7d |
