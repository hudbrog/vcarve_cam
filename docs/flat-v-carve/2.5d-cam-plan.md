...
`area` is `entire_stock` or an explicit setup-coordinate rectangle. An imported closed region is later work. Store four nonnegative area margins, a separate nonnegative entry and exit overrun distance along the pass direction, and an **entry choice**. Also store top/bottom heights, stepdown, stepover, pass angle, zigzag/one-way pattern, and milling assignment.

The entry choice is where every pass descends, and it is the user's: `min` or `max` is the low or high end of the pass axis (X at 0 degrees, Y at 90), `at` is an explicit position on that axis in setup coordinates, and `alternate` flips the end with the depth layer. `min` is the default; `alternate` is the pre-2026-09-15 behaviour and the only variant that needs clearance at both ends. `at` is the same fact as a travel value stated in coordinates — resolve it to the distance from the coverage edge — and a position inside the pass's own span is refused with `FACE_ENTRY_INSIDE_COVERAGE`, naming both allowed positions, because a pass starting there would leave the strip behind it unswept. A pass that *continues* its layer (a zig-zag link) is not an entry: it begins where the previous pass ended, so the entry travel applies only where the cutter descends.
...
Derive the maximum allowed envelope from settings and the pass pattern, independently of generated paths: expand the requested coverage by the largest travel the chosen entry mode actually uses at each end of the pass axis, then dilate by the cutter radius plus the numerical reserve. With one entry the envelope is asymmetric — the entry end reserves the entry travel, the other end only the exit travel — and a generated path must fit inside it. A buggy candidate path cannot authorize itself by expanding the allowed envelope to match its own motions.
...
Travel overrun is measured to the **cutter centre**, along the pass direction, past the coverage boundary. It is travel, not a coverage request. It belongs to the side a pass starts from, and every descent into uncleared material in one face operation happens at the one entry the user chose — a program that plunges at a different end on the next depth layer defeats the clearance the user verified, which is the defect the 2026-09-13 field report and the 2026-09-15 follow-up found.

The plan is a **continuous motion chain**: within a stage every motion starts where the previous one ended, and the travel between depth layers (or between passes) is a motion of its own. The post emits one block per motion endpoint, so a transition the plan leaves out becomes a straight move from wherever the tool really is; `PLAN_MOTION_DISCONTINUITY` is a basic check, and the entry-safety rule below measures a descent from the position the machine is actually at.

An entry is only a plunge when the cutter's sweep overlaps stock *material*. A cutter that merely touches the stock boundary has zero overlap, so descending there is a side entry and any tool may make it. A tool whose sweep would descend inside the stock must be marked `plunge_capable = true`, or the operation is reported incomplete: `FACE_ENTRY_UNSAFE` when the tool is marked as unable to plunge (the reason names the end, the position the cutter clears the stock from and the travel that reaches it), or the missing `assignment.tool.capabilities.plunge_capable` field when the answer has never been given. The same rule is enforced over the whole plan by the basic checks (`PLAN_ENTRY_UNVERIFIED`), so no planner can emit an unauthorized descent: a motion that lowers Z into the material must be clear of the stock, must re-enter space its own stage already cut, or must be authorized by the tool's declared capability — and a rapid never is.

The stock rectangle is the material. The entrance test is a distance test against `setup.stock.xy`, so a blank larger than the modelled stock is not described and the panel must say which rectangle the verdict used rather than guess at a bigger one.
...
4. Include first/last rows and extend row endpoints enough that the union of cutter sweeps covers the rectangle, including corners. Add the requested travel overrun after establishing coverage: the entry travel at the end a pass descends at, the exit travel at the end it leaves.
...
7. For one-way cutting, retract between rows. For zigzag, a cutting-height cross-row link is allowed only as a checked intended cut within this layer's allowed envelope, or a proved air move. Otherwise retract. Every retract, travel and descent is an explicit motion, so the chain never skips a position; between depth layers the tool retracts, travels to the chosen entry at the clearance plane and descends there, which is the only place a descent may happen.
...
If the entry cutter footprint reaches no stock material, descent there does not require plunging into material. For a partial interior face with no such entry, require a supported plunge/ramp and its feed, and require the tool to be marked able to make it. Being outside the requested face rectangle is not equivalent to being outside stock.
...
A near-reversal carries no swivel direction only when both of its legs are resolvable. A reversal whose shorter leg lies inside the planner's declared near-zero segment budget (section 12.4 step 2) is a flattening or rounding artifact at a smooth junction, not a corner: clean the degenerate segment and cut through it instead of rejecting the chain. Only a reversal with two resolvable legs is reported as ambiguous, and every cleaning stays inside the error share the planner declares for it.
...
The viewport draws the stock rectangle, the artwork and the generated toolpath
in **one display frame**, settled before generation output is normalized: the
frame is the union of the stock rectangle, the artwork and the executed
motions. A path that travels outside the stock widens that frame and moves the
camera; it never changes the scale of the artwork or of the stock block, which
must stay in the same relative position on screen before and after generation.
...
Retain the tiled heightfield for the supported milling surfaces. Initialize it from physical stock, including nonzero XY minimums. At cells emptied to stock bottom, render an actual hole/empty cell rather than an opaque bottom face that hides a cut-through slot. Preserve a visible stock side/bottom where material exists: the four outer walls belong to the **remaining** material, so each wall quad runs from its own boundary cell's machined surface down to the stock bottom, an untouched boundary cell still shows the full stock thickness, and a cell cut through leaves a real gap in the side. Never stand a wall at the original stock height above material that has been removed — a faced plate has to read as a plate, not as a tray with the original envelope around it. Detached pieces remain in place.
...
The next step of this display model — interior walls, per-cell operation/tool
identity, colouring modes, depth gradients, toggles and a translucent stock — is
planned in [stock-display-plan.md](stock-display-plan.md). That plan keeps the
heightfield authoritative, derives walls in the display process instead of
transporting them, and fixes the identity packing and style contracts before
any of it is built.
