...
**Addendum:** this section records the original web UI implementation route. The new shared Rust UI is governed by the [UI plan](2.5d-cam-ui-plan.md) and section 22.9. Keep the machining/display distinctions below, but do not rebuild these controls in the existing React UI to complete revised F3.
...
The incumbent React/Three.js UI is a compatibility client. Do not require implementing new knife forms or rebuilding its navigation to finish a backend slice. Preserve its advertised schema-4/`ui-8` behavior during migration through an explicit adapter, or explicitly gate unsupported newer documents. Never accept a collection job and silently send only its first source to an old route. Keep CLI and HTTP usable even if a native egui client eventually uses an in-process adapter.
...
Detailed UI implementation milestones and the egui/renderer prototype remain in the [UI plan](2.5d-cam-ui-plan.md). Backend contract design is authorized now; choosing dependencies or declaring the prototype successful is not part of this documentation task. General pocketing and other G operations remain optional and do not gate H1-H6.
...
| Contracts and artifact lifecycle | `cam-service/src/sequence.rs`, native sequence/task routes in `cam-server`, `cam-wasm/src/lib.rs`, CLI sequence commands. |
