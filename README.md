...
M0–M5 implement SVG jobs, endmill clearing, V-bit finishing/rest machining, combined stock previews, and bounded continuous stock verification. M6 adds LinuxCNC output with explicit machine profiles and numeric readback; actual controller validation remains pending. The M7 browser workflow is implemented: import-to-export runs in the local UI with background planning, stock slices, gated export, a local tool library, and a labeled 3D stock simulator. The same UI also builds statically with the engine compiled to WebAssembly for browser-only hosting. **That React workspace UI is now deprecated**: it is no longer built or tested, and the GUI is the `cam-gui` crate (native window plus a browser build) described in the [2.5D CAM UI plan](docs/flat-v-carve/2.5d-cam-ui-plan.md).
